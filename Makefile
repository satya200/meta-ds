...
QUEUE = $(PWD)/meta-ds/queue/src/queue_array.o
...
all:
	...
	$(MAKE) -C $(PWD)/meta-ds/queue/
	$(MAKE) -C $(PWD)/meta-ds/bst_tree/

lib : ${SINGLE} ${DOUBLE} ${DOUBLE_OPT} ${STACK} ${BST}
	${CC} -shared -o libDataStructure.so ${SINGLE} ${DOUBLE} ${DOUBLE_OPT} ${STACK} ${QUEUE} ${BST} ${CFLAGS}
...
clean:
	...
	$(MAKE) -C $(PWD)/meta-ds/queue/ clean
